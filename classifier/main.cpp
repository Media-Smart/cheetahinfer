...
samplesCommon::OnnxSampleParams initializeSampleParams(const samplesCommon::Args& args)
{
    ...
    params.engine_fp = "models/resnet18-v1-7.engine";
...
int main(int argc, char** argv)
{
    ...
    gLogInfo << "Building and running a GPU inference engine for Onnx ResNet18" << std::endl;

	cheetahinfer::Timer timer;
    classifer.build(false);
	for(int i=0; i<1; i++)
	{
		timer.start();
		if (!classifer.infer("data/cat.jpg"))
		{
