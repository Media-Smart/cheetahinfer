...
void RetinaNet::addPlugin(SampleUniquePtr<nvinfer1::INetworkDefinition> &network)
{
    ...
    for (int i = 0; i < nb_outputs / 2; i++) {
        ...
        int scale = input_dims.d[2] / outputDims.d[2]; // h
        auto decode_plugin = retinanet_plugin::DecodePlugin(score_thresh_, top_n_, anchors_[i], scale);
    ...
    // Add NMS plugin
    auto nmsPlugin = retinanet_plugin::NMSPlugin(nms_thresh_, detections_per_im_);
...
{
    const int output_size = output_dims_vec_[0].d[1];
    ...
    for (int i = 0; i < output_size; i++)
    {
        if (scores[i] > 0.9 && 0 == int(classes[i]))
        {
            ...
            sample::gLogInfo << "Found box {" << x1 << ", " << y1 << ", " << x2 << ", " << y2
...
bool RetinaNet::infer(const std::vector<std::string>& fps)
{
